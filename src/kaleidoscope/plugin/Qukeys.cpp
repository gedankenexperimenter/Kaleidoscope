...
EventHandlerResult Qukeys::onKeyswitchEvent(Key& key,
                                            KeyAddr k,
                                            uint8_t key_state) {
...
bool Qukeys::isQukey(KeyAddr k) {
  ...
  for (uint8_t i{0}; i < qukeys_count; ++i) {
    Qukey qukey = cloneFromProgmem(qukeys[i]);
    if (qukey.addr == k) {
...
bool Qukeys::releaseDelayed(uint16_t overlap_start, uint16_t overlap_end) const {
  ...
  return (elapsed_time < uint16_t(release_timeout));
}


// Helper function for `beforeReportingState()`. In order to restore a key that
// have been physically released, but whose release event has not yet been
