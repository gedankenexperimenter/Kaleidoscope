...
#include "kaleidoscope/EventQueue.h"
...
class Qukeys : public kaleidoscope::Plugin {
 ...
 public:
  ...
  Qukey const * qukeys{nullptr};
  uint8_t qukeys_count{0};
 ...
 private:
  ...
  EventQueue<queue_capacity_> event_queue_;
...
// guarantee that the count is set correctly.
#define QUKEYS(qukey_defs...) {                                                 \
  static kaleidoscope::plugin::Qukey const qk_table[] PROGMEM = { qukey_defs }; \
